	...
	if exists == False:
		...
		if rowCount == 0:
			...
			if retval['is_blacklisted'] == True:
				...
				rowCount, contentData, exists = lib.db.queryDatabase("SELECT * FROM {id}_contents".format(id=message.channel.id), client, channel=message.channel, checkExists=True, tablename="{id}_contents".format(id=message.channel.id), closeConn=True)
