...
async def createChroniclerChannel(client, message, createNew=True):
    ...
    channelCreator = discord.ChannelPermissions(
        target=message.author, overwrite=user_perms)
    chronicler = discord.ChannelPermissions(
        target=client.user, overwrite=user_perms)

    chroniclerChannel = None

    if createNew == True:
        #Create the New Channel
        chroniclerChannel = await client.create_channel(
            message.server, channelName, everyone, channelCreator, chronicler)
    else:
    ...
    lib.db.queryDatabase(
        "CREATE TABLE {id}_contents (entry_id INT AUTO_INCREMENT PRIMARY KEY, entry_type VARCHAR(255) NOT NULL DEFAULT(\"In-Character\"), char_count INT NOT NULL, word_count INT NOT NULL, entry_owner TEXT, entry_editted MEDIUMTEXT, entry_original MEDIUMTEXT)"
