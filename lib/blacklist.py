...
async def blacklistChronicle(message, client):
  ...
  #User confirmation is made by adding their channel id at the ned of the message
	if value != message.channel.id:
		...
		lib.db.queryDatabase("UPDATE chronicles_info SET is_blacklisted = TRUE WHERE channel_id={id};".format(id=message.channel.id), client, message=message, checkExists=True, tablename="chronicles_info", commit=True, closeConn=True)
