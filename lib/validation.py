...
def checkIfCanPost(message, client):
	rowCount, retval, exists = lib.db.queryDatabase("SELECT is_blacklisted,is_closed FROM chronicles_info WHERE channel_id={id}".format(id=message.channel.id), checkExists=True, tablename="chronicles_info", getResult=True, closeConn=True)
